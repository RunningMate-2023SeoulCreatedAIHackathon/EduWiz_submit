...
function showLoadingBar() {
  ...
  toast.id="loading_toast";
  toast.textContent = "위즈가 문제 생성중입니다.";
  console.log('토스트창');
...
function hideLoadingBar() {
  ...
  const contentArea = document.querySelector(".content_area");
  if (loadingBar) {
    contentArea.removeChild(loadingBar);
  }
  const toast = document.querySelector('#loading_toast');
  if (toast) {
    console.log('토스트창 삭제');
    contentArea.removeChild(toast);
  }
...
async function createChatBox(status) {
  ...
  if (status === "notLoggedIn") {
    ...
    await getUserInfo().then(async (result) => {
      ...
      progress = await calculateLevel(result.exp);
...
function createChatImage(imgUrl, lectureUrl, sender) {
  const link = document.createElement("a"); //a 요소 생성
  link.href = lectureUrl;

  const chatImage = document.createElement("img"); //img 요소 생성

  chatImage.classList.add("chat-image");
  chatImage.classList.add(sender);

  chatImage.src = imgUrl; //img에 src 속성 추가(이미지 링크)

  link.appendChild(chatImage); //img 요소를 a 요소 안에 넣기

  //foreignObject 태그 안에 이미지 추가
  document.querySelector("#chat-area").appendChild(link);
  document
    .querySelector("#chat-area")
    .scrollTo(0, document.querySelector("#chat-area").scrollHeight);
}
  ...
  const result = await new Promise((resolve, reject) => {
    chrome.storage.sync.get(["user"], (response) => {
      ...
      for (const fieldname in fields) {
        ...
        response.user[fieldname] = value;
        console.log(
          `now result.user ${fieldname} is ${response.user[fieldname]}`
        );
      }
...
async function calculateLevel(exp) {
  ...
  if (exp == undefined) {
    ...
    level = 1;
    progress = (exp - 1000) / 11;
  } else if (exp < 3300) {
    level = 2;
    progress = (exp - 2100) / 12;
  } else if (exp < 4600) {
    level = 3;
    progress = (exp - 3300) / 13;
  } else if (exp < 6000) {
    level = 4;
    progress = (exp - 4600) / 14;
  } else if (exp < 7600) {
    level = 5;
    progress = (exp - 6000) / 16;
  } else if (exp < 9400) {
    level = 6;
    progress = (exp - 7600) / 18;
  } else if (exp < 11400) {
    level = 7;
    progress = (exp - 9400) / 20;
  } else if (exp < 13900) {
    level = 8;
    progress = (exp - 11400) / 25;
  } else if (exp < 16900) {
    level = 9;
    progress = (exp - 13900) / 30;
  } else {
    level = 10;
      ...
      function (response) {
        ...
        if (response.gpt_recommend) {
          ...
          for (let i = 0; i < recommendList.length; i++) {
            ...
            createChatImage(recommendList[i][3], recommendList[i][4], "wiz");
          }
        } else {
          console.log("No response");
        }
...
async function showResults() {
  ...
  parent.innerHTML = `
     ...
     <div class="result-card">
       ...
       <p>경험치: ${xp}xp</p>
       <div class="level-bar">
         <div class="progress" style="width: ${(xp / 16900) * 100}%"></div>
       </div>
     </div>
   `;
  // ----------------------------
